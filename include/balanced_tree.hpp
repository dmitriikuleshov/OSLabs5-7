...
class BalancedTree {
      ...
      public:
        ...
        int IDMinimalHeight(int height, int current_height) {
            if (height < current_height) {
                ...
                if (left != NULL && left->available == true) {
                    current_id =
                        left->IDMinimalHeight(height, (current_height + 1));
                }
                if (right != NULL && right->available == true &&
                    current_id == -2) {
                    current_id =
                        right->IDMinimalHeight(height, (current_height + 1));
                }
                return current_id;
            } else {
                if (left == NULL || right == NULL) {
  ...
  public:
    std::set<int> ids;
    BalancedTree() { root = new BalancedTreeNode(-1); }
    bool Exist(int id) {
    ...
    void AvailabilityCheck(int id) { root->CheckAvailability(id); }
    int FindID() {
        int h = root->MinimalHeight();
        return root->IDMinimalHeight(h, 0);
    }
    void AddInTree(int id, int parent) { root->AddInNode(id, parent, ids); }
    void RemoveFromRoot(int idElem) { root->Remove(idElem, ids); }
    ~BalancedTree() {
